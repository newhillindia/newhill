...
export default function SignIn() {
  ...
  const onSendOTP = async (data: { email: string; language: string }) => {
    setIsLoading(true);
    try {
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            {isOTPMode ? 'Enter the OTP sent to your email' : 'Or use your email and password'}
          </p>
        </div>

        {!isOTPMode ? (
          <form className="mt-8 space-y-6" onSubmit={signInForm.handleSubmit(onSignIn)}>
          ...
          <form className="mt-8 space-y-6" onSubmit={otpForm.handleSubmit(onVerifyOTP)}>
            ...
            <div className="space-y-3">
              {!otpSent ? (
                <button
                  type="button"
                  onClick={() => onSendOTP({ email: otpForm.getValues('email'), language: 'en' })}
                  disabled={isLoading || !otpForm.watch('email')}
